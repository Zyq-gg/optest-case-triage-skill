...
| PyTorch 编译仓 | `<compile-repo>` | `<branch>/<HEAD>` | `<未使用/必须编译源码记录镜像/既有 build 状态>` | `源码镜像/既有 build 产物不提交` |
...
- 必须编译时记录编译仓同步文件和基线；不执行 PyTorch 源码构建，明确记录按规则未编译以及新二进制 runtime 验证未覆盖；
...
- 测试源码来自代码记录仓、实际 runtime 来自安装验证仓；不得把安装验证仓或编译仓既有二进制的结果归因于当前未编译 patch；
- 静态检查；
- 未运行范围、blocker 和残余风险；
- 安装验证仓保留修改的路径和当前状态。
