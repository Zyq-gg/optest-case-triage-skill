...
| --- | --- | --- | --- | --- | --- |
| 编译仓 | `<compile-repo>` | 仅同步必须编译的源码并作记录/static validation；不执行 PyTorch 源码编译 | `<branch>/<HEAD>` | `<未使用/源码镜像/既有 build 状态>` | `否` |
| 代码记录仓 | `<code-record-repo>` | 保存全部源码/test 修改并提供 pytest test，生成 Git diff、commit 和 push | `<branch>/<HEAD>` | `<状态>` | `是，唯一提交来源` |
| 安装验证仓 | `<Path(torch.__file__).resolve().parent>` | pytest 实际导入的 runtime；承接 Python runtime 验证，不接收本 skill 新构建的二进制 | `<torch version>` | `<validation-only 状态>` | `否；允许保留有效验证修改` |
...
- 若修改必须编译，记录编译仓同步了哪些源码和基线，并明确写 `按规则未执行 PyTorch 源码编译，未完成新二进制 runtime 验证`；不得把既有 build/安装产物的结果归因于当前 patch；
...
- 测试源码必须来自哪个代码记录仓、实际 runtime 来自哪个安装验证仓；必须编译的 patch 需明确实际 runtime 仍是既有产物，不能声称完成新二进制验证；
- 编译命令、安装/同步命令及其结果（如有）；
...
| 仓库角色 | 路径 | 用途 | branch/HEAD 或版本 | dirty/验证状态 | 是否可提交 |
| --- | --- | --- | --- | --- | --- |
| 编译仓 | `<compile-repo>` | 仅同步必须编译的源码并作记录/static validation；不执行 PyTorch 源码编译 | `<branch>/<HEAD>` | `<状态>` | `否` |
...
<测试源码来自代码记录仓、runtime 来自安装验证仓；如有，记录 Python 文件同步、必须编译源码在编译仓的记录副本，以及按规则未执行构建的验证边界和可保留的 validation-only 修改>
